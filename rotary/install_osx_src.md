...
Tools and dependencies for Rotary can be installed using the [Homebrew package manager](https://brew.sh/). After installing Homebrew, add the `osrf/simulation` to Homebrew tap to be able to install prebuilt dependencies.
...
Rotary is compatible with Python 3. Install the latest version from Homebrew:
...
All the sources of gazebo-rotary are declared in a yaml file. Download
it to the workspace:

```bash
curl -OL https://raw.githubusercontent.com/gazebo-tooling/gazebodistro/master/collection-rotary.yaml
```
...
```bash
brew install assimp boost bullet cli11 cmake cppzmq dartsim doxygen eigen fcl ffmpeg flann freeimage freetype gdal gflags google-benchmark ipopt jsoncpp libccd libyaml libzzip libzip nlopt ode open-scene-graph ossp-uuid ogre1.9 gz-rotary-ogre2.3-vendor pkg-config protobuf qt@6 qwt rapidjson ruby tbb tinyxml2 urdfdom zeromq
```
