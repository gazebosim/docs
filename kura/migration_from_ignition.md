...
- Branch names for versions of Gazebo libraries targeting releases before Kura
    - e.g. `ign-cmake2`
- Some links
...
In order to support side-by-side installs of `Kura` and `Fortress`, the `ign` CLI executable (and the `gazebo` verb) will not be installed for `Kura`.
This means that unless `Fortress` is also installed alongside `Kura`, CLI usage has to be migrated.
...
All internal references to `ign` or `ignition` that pre-Kura versions of Gazebo don't rely on have been migrated.
