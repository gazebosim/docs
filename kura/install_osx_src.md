...
Tools and dependencies for Kura can be installed using the [Homebrew package manager](https://brew.sh/). After installing Homebrew, add the `osrf/simulation` to Homebrew tap to be able to install prebuilt dependencies.
...
Kura is compatible with Python 3. Install the latest version from Homebrew:
...
All the sources of gazebo-kura are declared in a yaml file. Download
it to the workspace:

```bash
curl -OL https://raw.githubusercontent.com/gazebo-tooling/gazebodistro/master/collection-kura.yaml
```
...
```bash
vcs import < collection-kura.yaml
```
