...
Generally speaking, you should still be able to use either the Ignition counterpart or Gazebo counterpart for **most things** if you are using Harmonic, due to explicit tick-tocking logic written in the stack.
...
- Branch names for versions of Gazebo libraries targeting releases before Harmonic
    - e.g. `ign-cmake2`
- Some links
...
In order to support side-by-side installs of `Harmonic` and `Fortress`, the `ign` CLI executable (and the `gazebo` verb) will not be installed for `Harmonic`.
This means that unless `Fortress` is also installed alongside `Harmonic`, CLI usage has to be migrated.
...
All internal references to `ign` or `ignition` that pre-Harmonic versions of Gazebo don't rely on have been migrated.
